...
from rqdatac.services.future import get_dominant, get_commission_margin, get_dominant_price
...
trading_days_filename = os.path.dirname(__file__)+"/data/trading_days.json"

domain_contracts_filename=str(Path.home().joinpath(".vntrader/data/json/domain_contracts.json"))
contracts_rule_filename = str(Path.home().joinpath(".vntrader/data/json/contracts_rule.json"))

domain_contracts_dict = load_json(domain_contracts_filename)
domain_contracts_rule = load_json(contracts_rule_filename)
...
def update_trading_days_json():
    """从rqdata获取交易日列表"""
    datafeed: BaseDatafeed = get_datafeed()
    datafeed.init()

    start_day = '2010-01-01'
    three_years_end = datetime.strftime(datetime.now().date().replace(day=31,month=12,year=datetime.now().date().year+3),'%Y-%m-%d')
    trading_day_list_from_rq = [datetime.strftime(date, "%Y-%m-%d") for date in get_trading_dates(start_day,three_years_end)]
    furthest_trading_date_from_rqdata = trading_day_list_from_rq[-1]
    furthest_date_in_the_month = datetime.strftime((datetime.strptime(furthest_trading_date_from_rqdata, "%Y-%m-%d")+ relativedelta(months=+1)).replace(day=1) - timedelta(days=1),'%Y-%m-%d')
    day_range_list = date_range(start_day,furthest_date_in_the_month)
    trading_days_dict = {key:True if key in trading_day_list_from_rq else False for key in day_range_list}
    save_json(trading_days_filename, trading_days_dict)
    print(f"已更新trading_days_json")
    rqdatac.reset()
    return


#更新主力合约json
def update_domain_contract():
    """更新主力合约json"""
    datafeed = get_datafeed()
    datafeed.init()
    contractRuleDict = pd.read_csv(os.path.dirname(__file__)+"/data/contractRule.csv", index_col='symbolHead',encoding='gbk').to_dict(orient='index')
    contractRuleDict.pop('IO', None)
    contractRuleDict.pop('MO', None)
    domain_dict = {}
    tradeday_now = generate_trading_day(datetime.now())
    for key in contractRuleDict.keys():
        exchange = contractRuleDict[key]['exchange']
        try:
            contract = get_dominant(underlying_symbol = key, start_date = tradeday_now,rank=1)[-1]
        except:
            contract = get_dominant(underlying_symbol = key, start_date = tradeday_now-timedelta(days=1),rank=1)[-1]
        new_contract = to_CTP_symbol(contract)
        domain_dict[key] = new_contract
    save_json(domain_contracts_filename, domain_dict)
    rqdatac.reset()
    print(f"主力合约已更新")
...
class TrendFeatureArrayManager(ArrayManager):
    """
    For:
    ...
    def trend_feature(self, inflection_point_start: str, inflection_point_end: str = '2029-12-31') -> Tuple[
        ...
        if daily_count > 1:
            ...
            if self.open_interest[date_range[0]] > 0:
                range_holding_change_rate = (self.open_interest[date_range[-1]] - self.open_interest[date_range[0]]) / \
                                            self.open_interest[date_range[0]]
            else:
